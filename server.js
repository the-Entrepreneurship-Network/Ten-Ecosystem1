...
// Self-hosted xterm.js (F5 coding-challenge terminal) — serves the actual
// npm package files directly instead of relying on a CDN.
...
// NOTE: this loops over one student at a time. A single student with bad/edge-case
// data (e.g. malformed joiningDate) throwing inside the loop used to abort the whole
// function — which silently blanked the "Overall" leaderboard (many students, higher
// chance of hitting one bad record) while "My Domain" (few students) kept working.
// Each student is now isolated in its own try/catch so one bad record is skipped
// and logged instead of blanking the entire leaderboard.
async function _buildLeaderboard(filter, limit){
    ...
    for(const s of students){
        try{
            ...
            console.log("[leaderboard] skipped student due to error:", s.employeeId, err && err.message);
        }
    }
