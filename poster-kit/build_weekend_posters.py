#!/usr/bin/env python3
"""
Render the fourteen posters the weekend autopilot attaches to its posts.

Run once, commit the output, forget about it. That is the point: the alternative
is a headless browser running on the production box at ten past ten on a
Saturday morning with nobody watching it fail, to draw a poster whose every
word is a constant. The facts on these are the programme's facts — Remote,
2-6 Months, Unpaid, freshers welcome — and they do not change between weekends,
so the poster does not need to be drawn between weekends either.

    python poster-kit/build_weekend_posters.py

Output goes to public/assets/linkedin-posters/<slug>.jpg, where <slug> matches
the domain slugs in services/v2/linkedin/weekendPost.js. JPEG rather than PNG
...
Nothing here invents a number. There is no stipend figure, no opening count and
no batch on these posters, because nobody has stated one for the weekend run —
and a plausible figure nobody approved, printed on a live job ad, is the most
...
# looks for and must stay in step with DOMAINS in weekendPost.js; the pairing is
...
def job_for(name, slug, role, hero, workdir):
    ...
    job = {
        ...
        "headline": {
            ...
            "verb": "HIRING!",
            "role": role,
            "blurb": "You are on an industry-level project from week one, with a coordinator "
                     "reviewing your work — and you finish with a portfolio you can "
        ...
        "facts": [
            ...
            {"icon": "globe",     "label": "Mode",     "value": "Remote"},
            {"icon": "clock",     "label": "Duration", "value": "2-6 Months\nFlexible"},
            {"icon": "bulb",      "label": "Skills",   "value": "No Prior\nSkill Needed"},
            {"icon": "users",     "label": "Openings", "value": "Rolling\nIntake"},
        ],
        "eligibility": {"heading": "ELIGIBILITY", "items": list(ELIGIBILITY)},
        # Unpaid, stated on the poster and not only in the caption. Burying it
        # is how a fresher finds out after they have already started.
        "salary": {"label": "STIPEND", "value": "Unpaid", "note": "(Experience, certificate and LOR)"},
        "cta": {
            # The template sets `quote` in quotation marks — it was drawn for
            # COMMENT "INTERESTED" — so the phrase put there has to read as
            # something being quoted rather than as a stray noun.
            "kicker": "USE THE",
            "quote": "APPLY LINK",
            "tail": "IN THIS POST.",
            "sub": "CERTIFICATE | LETTER OF RECOMMENDATION | REAL PROJECT WORK.",
            "accent_note": "Hands-On\nFrom Week One",
        },
        "footer": {
            "left": "We run <b>virtual internships</b> and <b>early-career projects</b> across "
                    "all <b>14 domains</b> we teach.",
...
def main():
    ...
    workdir = Path(tempfile.mkdtemp(prefix="ten-weekend-"))
    ...
    # The autopilot looks up posters by the slug in weekendPost.js. If the two
    ...
    src = (REPO / "services" / "v2" / "linkedin" / "weekendPost.js").read_text(encoding="utf-8")
    import re
    js_slugs = set(re.findall(r"slug:\s*'([a-z]+)'", src))
    ours = {slug for _, slug, _ in DOMAINS}
    if js_slugs != ours:
        print(f"slug mismatch with weekendPost.js: only in JS {sorted(js_slugs - ours)}, "
              f"only here {sorted(ours - js_slugs)}", file=sys.stderr)
        sys.exit(1)
    print(f"slugs match weekendPost.js ({len(ours)} domains)")
