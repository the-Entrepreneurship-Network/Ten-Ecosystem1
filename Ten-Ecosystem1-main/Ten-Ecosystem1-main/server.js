...
function wrapModelWithFileFallback(model) {
    ...
    const originalEstimatedDocumentCount = model.estimatedDocumentCount;
    model.estimatedDocumentCount = function(options) {
        if (!checkMongoStatus()) {
            return new FallbackQuery(async () => {
                return runCountDocumentsFallback({});
            });
        }
        const mongooseQuery = originalEstimatedDocumentCount.apply(this, arguments);
        const originalExec = mongooseQuery.exec;
        mongooseQuery.exec = function() {
            return originalExec.apply(this, arguments).catch(err => {
                if (isNetworkError(err)) {
                    console.warn(`[DB FALLBACK] estimatedDocumentCount failed with network error: "${err.message}". Falling back...`);
                    return runCountDocumentsFallback({});
                }
                throw err;
            });
...
// Also patch all models that were already compiled before this override (e.g. Student model required in studentPropagation)
const compiledModels = mongoose.modelNames ? mongoose.modelNames() : Object.keys(mongoose.models || {});
for (const modelName of compiledModels) {
    try {
...
app.get('/api/public/contributors', async (req, res) => {
    try {
        ...
        if (mapped.length === 0) {
            // Default developer card for local and fresh environments
            mapped = [{
                name: "Devraj Shinde",
                domain: "SOFTWARE ENGINEER",
                contribution: "Integrated coin marketplace & secure payments, polished responsive UI, dashboards, and automated attendance.",
                photoUrl: "https://virtualinternships.entrepreneurshipnetwork.net/uploads/contributors/devraj-shinde.jpg"
            }];
        }
