...
router.post("/certificates/claim/:type", requireStudent, async (req, res) => {
    try {
        ...
        if (!paymentConfig.RAZORPAY_KEY_ID || !paymentConfig.RAZORPAY_KEY_SECRET) {
            return res.status(400).json({
                success: false,
                message: "Razorpay credentials are not configured. Please add RAZORPAY_KEY_ID and RAZORPAY_KEY_SECRET to your .env file."
            });
        }
        let orderId = null;
        let finalAmount = getCertificatePrice(type, student.tenure) * 100;
        ...
        try {
            ...
            finalCurrency = order.currency;
        } catch (rzpErr) {
            console.error("[CERT] Razorpay order creation failed:", rzpErr.message);
            return res.status(400).json({
                success: false,
                message: "Razorpay Order Creation Failed: " + rzpErr.message
            });
        }
