...
describe('routes/v2/linkedinAgent — POST /connect', () => {
  const TOKEN = 'AQ' + 'x'.repeat(200);

  beforeEach(() => {
    client.listAdministeredOrganizations.mockResolvedValue([
      { orgUrn: 'urn:li:organization:12345', role: 'ADMINISTRATOR', state: 'APPROVED' },
    ]);
    client.resolveOrganization.mockResolvedValue({
      orgUrn: 'urn:li:organization:12345', name: 'The Entrepreneurship Network', vanity: 'x',
    });
  });

  /*
   * This route accepts a credential that can post as the company. It is the
   * same privilege the OAuth flow grants, so it gets the same gate: HR and
   * admin and nobody else, including the staff roles that may read everything.
   */
  it('is HR and admin only', async () => {
    for (const role of ['coordinator', 'mentor', 'founder', 'student', 'investor', 'contractor']) {
      expect((await post('/connect', { token: TOKEN }, role)).status).toBe(403);
    }
    expect((await post('/connect', { token: TOKEN })).status).toBe(401);
    expect(LinkedInConnection.findOneAndUpdate).not.toHaveBeenCalled();
  });

  it('stores the token and the page once LinkedIn confirms them', async () => {
    const res = await post('/connect', { token: TOKEN }, 'hr');
    expect(res.status).toBe(200);
    expect(res.body.ok).toBe(true);
    expect(res.body.orgId).toBe('12345');

    const update = LinkedInConnection.findOneAndUpdate.mock.calls[0][1];
    expect(update.$set.accessToken).toBe(TOKEN);
    expect(update.$set.orgUrn).toBe('urn:li:organization:12345');
    expect(update.$set.connectedBy.role).toBe('hr');
  });

  /*
   * The whole reason the route asks LinkedIn first. A token stored as though
   * it worked puts the agent straight back into building posts and sending
   * none of them — the exact failure this feature already had once.
   */
  it('stores nothing when LinkedIn reports no administered pages', async () => {
    client.listAdministeredOrganizations.mockResolvedValue([]);
    const res = await post('/connect', { token: TOKEN }, 'hr');
    expect(res.status).toBe(400);
    expect(res.body.hints.join(' ')).toMatch(/Community Management API/);
    expect(LinkedInConnection.findOneAndUpdate).not.toHaveBeenCalled();
  });

  it('stores nothing when LinkedIn cannot be reached', async () => {
    client.listAdministeredOrganizations.mockRejectedValue(new Error('socket hang up'));
    expect((await post('/connect', { token: TOKEN }, 'hr')).status).toBe(502);
    expect(LinkedInConnection.findOneAndUpdate).not.toHaveBeenCalled();
  });

  it('refuses something that is not a token, without calling LinkedIn', async () => {
    expect((await post('/connect', {}, 'hr')).status).toBe(400);
    expect((await post('/connect', { token: '   ' }, 'hr')).status).toBe(400);
    expect((await post('/connect', { token: 'short' }, 'hr')).status).toBe(400);
    expect(client.listAdministeredOrganizations).not.toHaveBeenCalled();
  });

  /* Never guess which company to speak as. Two pages means ask. */
  it('asks which page when the token administers more than one', async () => {
    client.listAdministeredOrganizations.mockResolvedValue([
      { orgUrn: 'urn:li:organization:111' },
      { orgUrn: 'urn:li:organization:222' },
    ]);
    const res = await post('/connect', { token: TOKEN }, 'hr');
    expect(res.status).toBe(409);
    expect(res.body.pages.map((x) => x.orgId)).toEqual(['111', '222']);
    expect(LinkedInConnection.findOneAndUpdate).not.toHaveBeenCalled();

    const picked = await post('/connect', { token: TOKEN, orgId: '222' }, 'hr');
    expect(picked.status).toBe(200);
    expect(picked.body.orgId).toBe('222');
  });

  it('refuses a page the token does not administer', async () => {
    const res = await post('/connect', { token: TOKEN, orgId: '99999' }, 'hr');
    expect(res.status).toBe(400);
    expect(LinkedInConnection.findOneAndUpdate).not.toHaveBeenCalled();
  });
  ...
  it('never returns the token in any response', async () => {
    const ok = await post('/connect', { token: TOKEN }, 'hr');
    expect(JSON.stringify(ok.body)).not.toContain(TOKEN);

    client.listAdministeredOrganizations.mockResolvedValue([]);
    const bad = await post('/connect', { token: TOKEN }, 'hr');
    expect(JSON.stringify(bad.body)).not.toContain(TOKEN);

    const feed = await get('/feed', 'hr');
    expect(JSON.stringify(feed.body)).not.toContain(TOKEN);
  });

  it('clears the cached connection so the dashboard updates at once', async () => {
    await post('/connect', { token: TOKEN }, 'hr');
    expect(client.forgetConnection).toHaveBeenCalled();
  });
});

describe('routes/v2/linkedinAgent — POST /disconnect', () => {
  it('is HR and admin only, and empties the stored token', async () => {
    expect((await post('/disconnect', {}, 'coordinator')).status).toBe(403);
    expect((await post('/disconnect', {}, 'student')).status).toBe(403);

    const res = await post('/disconnect', {}, 'admin');
    expect(res.status).toBe(200);
    const update = LinkedInConnection.findOneAndUpdate.mock.calls[0][1];
    expect(update.$set.accessToken).toBe('');
    expect(update.$set.orgUrn).toBe('');
  });
});
